...
boot_alloc(uint32_t n)
{
	...
	if (n == 0)
		...
		if((uint32_t)nextfree > 0xFFFFFFFF) {
			panic("Out of memory: 0x%x\n", (uint32_t)nextfree);
		}
		cprintf("boot_alloc: n %u, nextfree 0x%x\n", n, (uint32_t)nextfree);
		return tmp;
	}
...
page_init(void)
{
	...
	// 1) 2)
	size_t pg_mpentry = MPENTRY_PADDR / PGSIZE;
	size_t i;
	for (i = 1; i < npages_basemem; i ++) {
		if(i == pg_mpentry) continue;
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
	// 3) 4)
	size_t k = PADDR(boot_alloc(0)) / PGSIZE;
	for(i = k; i < npages; i ++) {
		if(i == pg_mpentry) continue;
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
...
page_alloc(int alloc_flags)
{
	...
	// fill in '\0' if (alloc_flags & ALLOC_ZERO)
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	physaddr_t pa_s, pa_e;
	pa_s = ROUNDDOWN(pa, PGSIZE); pa_e = ROUNDUP(pa + size, PGSIZE);
	size = pa_e - pa_s;
	boot_map_region(kern_pgdir, base, size, pa_s, PTE_PCD | PTE_PWT | PTE_W);
	base = ROUNDDOWN(base + size, PGSIZE);
	return (void *)_base;
...
check_va2pa(pde_t *pgdir, uintptr_t va)
{
	...
	if (!(*pgdir & PTE_P))
		return ~0;
	if (*pgdir & PTE_PS) {
		return L_PTE_ADDR(*pgdir) | L_PGOFF(va);
	}
