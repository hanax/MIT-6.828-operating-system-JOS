...
sys_exofork(void)
{
	...
	memcpy((void *)(&(child_env->env_tf)), (void *)(&(curenv->env_tf)), 
...
sys_env_set_status(envid_t envid, int status)
{
	...
	struct Env * tmp_env;
	int r = envid2env(envid, &tmp_env, 1);
	if (r < 0)
		return r;

	tmp_env->env_status = status;
	return 0;
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	tmp_env->env_pgfault_upcall = func;
	return 0;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if ((perm & PTE_U) != PTE_U) return -E_INVAL;
	if ((perm & PTE_P) != PTE_P) return -E_INVAL;
	if ((perm & (~PTE_SYSCALL)) != 0) return -E_INVAL;

	struct PageInfo * pg = page_alloc(ALLOC_ZERO);
	if (pg == NULL) return -E_NO_MEM;
	r = page_insert(tmp_env->env_pgdir, pg, va, perm);
	if (r < 0) {
		page_free(pg);
		return r;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	r = page_insert(dstenv->env_pgdir, pg, dstva, perm);
	if (r < 0) return r;
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	page_remove(tmp_env->env_pgdir, va);
	return 0;
...
{	// LAB 4: Your code here.
	//cprintf("%d sys_ipc_try_send ing...\n", sys_getenvid());
	struct Env *dstenv;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch(syscallno) {
		...
		case SYS_env_set_priority:
			ret = sys_env_set_priority((envid_t)a1, (uint32_t)a2);
			break;
		default:
			ret = -E_INVAL;
	}
